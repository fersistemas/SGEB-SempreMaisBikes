...
if ( 
  ...
  ! empty ( $dados_usuario['senha'] ) 
	...
	$pdo_checa_user = $conexao_pdo->prepare('SELECT * FROM tb_login WHERE user = ? LIMIT 1');
	...
	if ( $dados_usuario['senha'] = $fetch_usuario['user_password'] = $fetch_usuario['user_password'] ) {
