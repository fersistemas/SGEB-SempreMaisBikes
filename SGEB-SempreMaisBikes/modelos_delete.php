...
if(!empty($_POST))
{
    ...
    $sql = "DELETE FROM tb_modelo where cod_modelo = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($id));
    Banco::desconectar();
			...
			<div class="card ">
				...
				<div class="card-body">				
					<div class="span10 offset1">
						<form class="form-horizontal" action="modelos_delete.php" method="post">
							<input type="hidden" name="id" value="<?php echo $id;?>" />
							<div class="alert alert-danger container_center">Deseja excluir?
							</div>
							<div class="form actions container_center">
								<button type="submit" class="btn btn-danger">Sim</button>
								<a href="modelos.php" type="btn" class="btn btn-default">Não</a>
							</div>
						</form>
					</div>	
				</div>
